    ...
    private = list(
      ...
      gen_synthpop_filename =
        function(mc_,
          ...
          if (mc_primer == 0L) mc_primer <- design_$sim_prm$n_primers
          return(
            list(
      ...
      gen_synthpop = # returns NULL. Writes synthpop on disk
        function(mc_,
          ...
          if (mc_ <= design_$sim_prm$n_primers &&
            ...
            # Ensure the file write is complete (size stable)
            sz1 <- file.size(filename_$primer)
            Sys.sleep(3)
            sz2 <- file.size(filename_$primer)
            while (sz1 != sz2) {
              sz1 <- file.size(filename_$primer)
              Sys.sleep(3)
              sz2 <- file.size(filename_$primer)
          ...
          # del columns stored in the primer but exclude year and pid for the
          # linkage and t2dm_dgn and ncc as they are updated from the primer
          nam <- c(nam,
      ...
      get_synthpop =
        function(mc_,
          ...
          dt[, bpmed_adherence := rBE(.N, design_$sim_prm$bpmed_adherence, 0.2)]
