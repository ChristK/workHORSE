...
## current pop number continue from 2042 - 2071 ####
population_after_2041 <- population_before_2041[year == 2041L]

for (i in 2042: 2071) {
  subset_data <- population_before_2041[year == 2041L]
  subset_data[, year := i]
  population_after_2041 <- rbind(population_after_2041, subset_data)
}

## all population ####
all_population <- rbind(population_before_2041, population_after_2041)
setkey(all_population, year, age, sex, LAD17CD)

write_fst(all_population, "./ONS_data/pop_size/pop_proj.fst", 100)
...
# 
# model_population<- glm(pops ~ age + sex + year + LAD17CD , family = gaussian(link = "log"), data = population_train)
